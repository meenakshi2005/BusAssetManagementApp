...
// const makeToday8am = () => { const d = new Date(); d.setHours(8,  0, 0, 0); return d; };
// const makeToday5pm = () => { const d = new Date(); d.setHours(17, 0, 0, 0); return d; };
...
//     if (sel) { setTempDate(sel); if (Platform.OS === 'android') setShowTime(true); }
//   };

//   const onTimeChange = (_, sel) => {
//     if (Platform.OS === 'android') setShowTime(false);
//     if (sel) {
//       const m = new Date(tempDate || value);
//       m.setHours(sel.getHours(), sel.getMinutes(), 0, 0);
//       onChange(m);
//     }
//   };
...
//         onPress={() => { setTempDate(value); setShowDate(true); }}
//         activeOpacity={0.7}
//       >
...
// //  BUS PICKER MODAL
// // ─────────────────────────────────────────────────────────────
// function BusPickerModal({ visible, busList, selected, onSelect, onClose }) {
//   return (
...
//           <Text style={styles.sheetTitle}>Select Bus</Text>
...
//           itemStyle={{ color: T.textPrimary, fontSize: 15 }}
//         >
//           <Picker.Item label="All Buses" value="" color={T.textPrimary} />
//           {busList.map((id) => (
//             <Picker.Item key={id} label={id} value={id} color={T.textPrimary} />
//           ))}
//         </Picker>
//       </View>
//     </Modal>
...
// //  STAT CARD
// // ─────────────────────────────────────────────────────────────
// function StatCard({ borderColor, iconBg, iconColor, icon, label, value, badgeBg, badgeTextColor, badgeLabel }) {
//   return (
...
//         <Text style={[styles.statBadgeText, { color: badgeTextColor }]}>{badgeLabel}</Text>
//       </View>
//     </View>
//   );
// }
...
//           <View style={styles.logInfoCell}>
//             <View style={[styles.logInfoDot, { backgroundColor: T.amberMid }]} />
//             <View>
...
//   // ── fetch ──────────────────────────────────────────
//   const fetchLogs = useCallback(async (newOffset = 0, isRefresh = false) => {
...
//   const applyFilters = () => { setOffset(0); setHasMore(true); setLogs([]); fetchLogs(0); };
//   const loadMore     = () => { if (hasMore && !loading) { const n = offset + limit; setOffset(n); fetchLogs(n); } };
//   const onRefresh    = () => { setRefreshing(true); setOffset(0); setHasMore(true); fetchLogs(0, true); };
...
//           <Text style={[styles.summaryValue, { color: '#22c55e' }]}>{totalEntry}</Text>
...
//           <Text style={[styles.summaryValue, { color: '#f59e0b' }]}>{totalExit}</Text>
//           <Text style={styles.summaryTitle}>Total Exit</Text>
//        </View>

//         {/* Occupancy */}
//         <View style={[styles.summaryCard, { borderTopColor: '#6366f1', marginRight: 0 }]}>
        
//           <Text style={[styles.summaryValue, { color: '#6366f1' }]}>{occupancy}</Text>
...
//           <TouchableOpacity style={styles.retryBtn} onPress={() => fetchLogs(0)}>
...
//               <Text style={[styles.emptySub, { marginTop: 14 }]}>Fetching logs…</Text>
//             </View>
//           )
//         }
//       />
      ...
      <View style={styles.headerPanel}>
        ...
        <View style={styles.filterRow}>
          ...
          <View style={styles.filterField}>
            ...
            <View style={[styles.filterInput, { paddingVertical: 0, paddingHorizontal: 0, overflow: 'hidden' }]}>
              <Picker
                selectedValue={selectedBus}
                onValueChange={(val) => setSelectedBus(val)}
                mode="dropdown"
                style={{ flex: 1, height: 42, color: T.textPrimary, backgroundColor: 'transparent' }}
                dropdownIconColor={T.textMuted}
              >
                <Picker.Item label="All Buses" value="" style={{ fontSize: 13, color: T.textPrimary }} />
                {busList.map((id) => (
                  <Picker.Item key={id} label={id} value={id} style={{ fontSize: 13, color: T.textPrimary }} />
                ))}
              </Picker>
            </View>
          </View>
