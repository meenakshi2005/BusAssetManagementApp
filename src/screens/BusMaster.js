...
export default function BusMaster() {
  ...
  // --- Time picker helpers -------------------------------------------------
  // Convert a stored "HH:MM" string into a Date object so the native picker
  // can show the right initial time. Falls back to "now" if empty/invalid.
  ...
  // Convert a Date object back into a "HH:MM" (24-hour) string for storage.
  ...
  const handleDepartureTimeChange = (event, selectedTime) => {
    // Keep open on iOS (inline/spinner), close on Android (dialog closes itself)
    setShowDeparturePicker(Platform.OS === "ios");
    if (event.type === "dismissed") return;
    if (selectedTime) {
      handleInputChange("departure_time", formatDateToTimeString(selectedTime));
    }
  };
  ...
  const handleArrivalTimeChange = (event, selectedTime) => {
    ...
    if (event.type === "dismissed") return;
    if (selectedTime) {
      handleInputChange("arrival_time", formatDateToTimeString(selectedTime));
    }
  };
  // --------------------------------------------------------------------------
            ...
            <View style={styles.inputRow}>
              <View style={styles.inputHalf}>
                ...
                >
                  <Text
                    style={{
                      color: formData.departure_time ? "#2d3748" : "#a0aec0",
                    }}
                  >
                    {formData.departure_time || "HH:MM"}
                  </Text>
                </TouchableOpacity>
                {showDeparturePicker && (
                  <DateTimePicker
                    value={parseTimeStringToDate(formData.departure_time)}
                    mode="time"
                    is24Hour
                    display="default"
              ...
              <View style={styles.inputHalf}>
                ...
                >
                  <Text
                    style={{
                      color: formData.arrival_time ? "#2d3748" : "#a0aec0",
                    }}
                  >
                    {formData.arrival_time || "HH:MM"}
                  </Text>
                </TouchableOpacity>
                {showArrivalPicker && (
                  <DateTimePicker
                    value={parseTimeStringToDate(formData.arrival_time)}
                    mode="time"
                    is24Hour
                    display="default"
